...
class DisplayNameFromMethodNameTest implements UnitTest {
    ...
    Stream<DynamicTest> defaultDataTable() {
        return Stream.of(
                ...
                Map.entry("when_call_someMethod_then_should_something", "when call 'someMethod' then should something"),
            ...
            .map(entry -> DynamicTest.dynamicTest(
                    "Method name \"%s\" should be converted to display name \"%s\"".formatted(entry.getKey(), entry.getValue()),
                    () -> {
