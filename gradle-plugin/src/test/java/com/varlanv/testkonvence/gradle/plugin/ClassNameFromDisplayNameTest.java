...
class ClassNameFromDisplayNameTest implements UnitTest {
    ...
    Stream<DynamicTest> defaultDataTable() {
        return Stream.of(
                        ...
                        Map.entry(
                                "Some good fixture name, which has comas, dots., question? marks, and exclamation marks!",
                ...
                .map(entry -> DynamicTest.dynamicTest(
                        "Display name \"%s\" should be converted to class name \"%s\""
                                .formatted(entry.getKey(), entry.getValue()),
                        () -> {
                            var expectedClassName = entry.getValue();
                            var displayName = entry.getKey();
                            var actualClassName =
                                    new ClassNameFromDisplayName(displayName, "AnyOriginalClassName").newName();
